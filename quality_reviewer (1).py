#!/usr/bin/env python3
"""
Quality Reviewer Application

Compares a source document (PDF/DOCX/TXT/CSV/XLSX/XLS) with a processed XML file based on a
configuration mapping. Produces a QC report (CSV/XLSX/JSON) and an optional HTML summary.

Usage:
  python quality_reviewer.py --source path/to/source.pdf \
                             --xml path/to/processed.xml \
...
# Allowed packages per environment constraints
from xml.etree import ElementTree as ET

# Optional imports guarded where necessary
try:
...
# New: generic table reader for CSV/Excel
...
# Python's re doesn't support \p{P}, implement simple substitute
...
def parse_date(s: str) -> Optional[datetime]:
    ...
    for fmt in COMMON_DATE_FORMATS:
        try:
            ...
            pass
    # Try loose digits like YYYYMMDD
...
class ComparisonSpec:
    ...
    tolerance: Optional[float] = None  # for numeric absolute tolerance
    pct_tolerance: Optional[float] = None  # for numeric percentage tolerance e.g., 1.0 means 1%
    date_tolerance_days: Optional[int] = None  # for date comparisons
    fuzzy_threshold: float = 0.9  # for fuzzy
...
def extract_from_xml(tree: ET.ElementTree, spec: XmlSpec) -> Optional[str]:
    # xml.etree supports basic XPath like .//Case/ID
    try:
...
def build_field_specs(cfg: Dict[str, Any]) -> List[FieldSpec]:
    ...
    for f in fields_cfg:
        ...
        fs = FieldSpec(
            ...
            source=SourceSpec(method=source_cfg.get('method'), pattern=source_cfg.get('pattern'), flags=source_cfg.get('flags', []), csv_column=source_cfg.get('csv_column')),
...
def generate_reports(results: List[Dict[str, Any]], outdir: str, base_name: str, formats: List[str], html_summary: bool = False) -> Dict[str, str]:
    ...
    if html_summary:
        ...
        for r in results:
            st = r.get('status')
            color = {'PASS':'#d1fae5','FAIL':'#fee2e2','WARN':'#fef9c3','NA':'#e5e7eb'}.get(st,'#ffffff')
            row = f"<tr style='background:{color}'><td>{html.escape(str(r.get('field')))}</td><td>{html.escape(str(r.get('source_value')))}</td><td>{html.escape(str(r.get('xml_value')))}</td><td>{html.escape(str(st))}</td><td>{html.escape(str(r.get('comparator')))}</td><td>{html.escape(str(r.get('note', '')))}</td></tr>"
...
def parse_args(argv: Optional[List[str]] = None):
    p = argparse.ArgumentParser(description='Quality Reviewer: compare source document with processed XML.')
    p.add_argument('--source', required=True, help='Path to source document (PDF/DOCX/TXT/CSV/XLSX/XLS)')
    p.add_argument('--xml', required=True, help='Path to processed XML')
    p.add_argument('--config', required=True, help='Path to JSON config defining fields and rules')
    p.add_argument('--outdir', default='qc_output', help='Output directory for reports')
    p.add_argument('--report-format', default='all', choices=['csv','xlsx','json','all'], help='Report format(s)')
    p.add_argument('--html-summary', action='store_true', help='Generate HTML summary')
    return p.parse_args(argv)

if __name__ == '__main__':
    args = parse_args()
    try:
        paths = run_qc(args.source, args.xml, args.config, args.outdir, args.report_format, args.html_summary)
